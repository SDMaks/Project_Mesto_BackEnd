...
routUser.get("/users", (req, res) => {
  sendJsonfiles("user.json", res); //в файле helpers есть фукции по чтению и обработке данных
});
/*************************** */
...
routUser.get("/users/:id", (req, res) => {
  ...
  nameStream.on("end", () => {
    ...
    res.send(findUser(userChunkObject, [req.params.id])); //берем функцию из helpers -> добавлем параметры и отпраляем пользователю
  });
});
